...
// enableProxyRegistry 通过注册表确保 ProxyEnable=1
func enableProxyRegistry() error {
	...
	defer key.Close()
	return key.SetDWordValue("ProxyEnable", 1)
}

// disableProxyRegistry 通过注册表确保 ProxyEnable=0
func disableProxyRegistry() error {
	key, err := registry.OpenKey(registry.CURRENT_USER,
		`Software\Microsoft\Windows\CurrentVersion\Internet Settings`,
		registry.SET_VALUE)
	if err != nil {
...
func SetGlobalProxy(proxyServer string, bypasses ...string) error {
	...
	if err := enableProxyRegistry(); err != nil {
		return fmt.Errorf("failed to enable proxy via registry: %v", err)
	}
...
func Off() error {
	...
	if err := disableProxyRegistry(); err != nil {
		return fmt.Errorf("failed to disable proxy via registry: %v", err)
	}
	return Flush()
}
